...
import com.jcirmodelsquad.tcjcir.JCIRTraincraft;
...
public class GuiGeometryCar extends GuiScreen {
        ...
        if (button.id == 1) {
           ...
           System.out.println(thing2);
            if (Boolean.valueOf(thing2))  {
                JCIRTraincraft.startMissionPacketChannel.sendToServer(new StartMissionPacket(theCar.getEntityId(), false));
            } else {
                JCIRTraincraft.startMissionPacketChannel.sendToServer(new StartMissionPacket(theCar.getEntityId(), true));
            }

        } else if (button.id == 2) {
            JCIRTraincraft.generateTrackReportChannel.sendToServer(new GenerateTrackReport(theCar.getEntityId(), 1));
        } else {
            JCIRTraincraft.generateTrackReportChannel.sendToServer(new GenerateTrackReport(theCar.getEntityId(), 2));
        }
    }
    @Override
    ...
    protected void keyTyped(char par1, int par2) {
        if (par2 == Keyboard.KEY_ESCAPE) {
            ...
            System.out.println(JCIRTraincraft.updateGeometryCarChannel==null);
            ...
            JCIRTraincraft.updateGeometryCarChannel.sendToServer(new UpdateGeometryCar(theCar.getEntityId(), railroadName.getText(), geometryCarName.getText(), railroadType.getText(), railroadStandard.getText()));
